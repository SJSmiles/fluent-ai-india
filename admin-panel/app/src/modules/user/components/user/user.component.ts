...
export class UserComponent extends RecordsListComponent<any> implements OnInit, OnDestroy {
  @ViewChild('createUserModel') createUserModel!: TemplateRef<any>;
  @ViewChild('changePasswordModel') changePasswordModel!: TemplateRef<any>;
  @ViewChild('confirmModal') confirmModal!: TemplateRef<any>;
  userModalRef: NgbModalRef | null = null;
  ...
  createUser() {
    ...
    this.userModalRef = this._modalService.open(this.createUserModel, {
      centered: true,
      size: 'lg',
      backdrop: 'static',
  ...
  updateUser(dropdown: NgbDropdown, user: any): void {
  ...
  changePassword(dropdown: NgbDropdown, user: any): void {
    this.selectedUserData = user;
    dropdown.close();
    this.passwordModalRef = this._modalService.open(this.changePasswordModel, {
      centered: true,
      size: 'md',
      backdrop: 'static',
      windowClass: 'custom-password-modal'
    });
  }
  ...
  toggleUserStatus(modal: NgbModalRef) {
    if (!this.selectedUserData) return;

    const newStatus = this.selectedUserData.status === 1 ? 0 : 1;
    ...
    this._userService
      ...
      .subscribe({
        next: (res: any) => {
          ...
          this.showSuccessToast(`User ${action} successfully!`);
          modal.close();
          this.getRecordsList();
        },
        error: (err) => {
          const action = newStatus === 1 ? 'activate' : 'deactivate';
          this.showErrorToast(err?.error?.message || `Failed to ${action} user`);
          this.hideLoader();
        },
  ...
  get bmbyEnabled(): boolean {
    if (this.currentUser?.user?.isSuperAdmin) {
      return this.selectedCompany?.bmbyConfig || false;
    }

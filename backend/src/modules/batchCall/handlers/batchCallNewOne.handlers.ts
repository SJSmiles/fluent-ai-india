...
import { createBatchCallPM } from './batchCallPM';
...
function validateContactsBatch(
  ...
  const bmbyIdSet = new Set<number>();
  ...
  for (let i = 0; i < contacts.length; i++) {
    ...
    try {
      ...
      if (!bmbyIdStr) {
        rowErrors.push('Client ID is required');
      }
      ...
      // Validate BmbyId
      let bmbyIdNum: number | null = null;
      if (bmbyIdStr) {
        bmbyIdNum = parseInt(bmbyIdStr);
        if (isNaN(bmbyIdNum) || !Number.isInteger(bmbyIdNum)) {
          rowErrors.push('Client ID must be a valid number');
          bmbyIdNum = null;
        }
      }
      ...
      if (bmbyIdNum && !isBlacklisted && bmbyIdSet.has(bmbyIdNum)) {
        rowErrors.push(`Duplicate Client ID '${bmbyIdNum}'`);
      }
      ...
      } else if (contactNumber && bmbyIdNum) {
        // Valid contact
...
async function validateContactSheetBatch(
  ...
  for (let i = 0; i < contacts.length; i++) {
    ...
    try {
      ...
      const bmbyIdNum = contactData.bmbyId;
      ...
      if (!bmbyIdNum) {
        rowErrors.push('Client ID is missing in contact data');
      }
  ...
  if (isContactSheet) {
    ...
    if (validContacts.length > 0) {
      const validData = validContacts.map((pc) => ({
        ...
        'Client ID': pc.validatedData?.bmbyid || '',
    ...
    for (let i = 0; i < normalizedContacts.length; i += BATCH_SIZE) {
      ...
      for (const processedContact of processed) {
        if (processedContact.status === 'success' && processedContact.validatedData) {
          ...
          const isDuplicateBmbyId = globalBmbyIdSet.has(contact.bmbyid);

          if (isDuplicateNumber || isDuplicateBmbyId) {
            const errors = [];
            if (isDuplicateNumber)
              errors.push(`Duplicate Phone Number '${contact.number}' found across file`);
            if (isDuplicateBmbyId)
              errors.push(`Duplicate Client ID '${contact.bmbyid}' found across file`);
...
export async function createBatchCallHandlerNew(request: any, reply: any) {
  try {
    const user = request.user;
    if (user.companyId.toString() !== PM_QUALI_COMPANY_ID) {
      const result = await createBatchCall(request, reply);
      return result;
    } else {
      const result = await createBatchCallPM(request, reply);
      return result;
    }
    ...
    return reply.status(500).send({
      ...
      message: 'An unexpected error occurred while processing the batch call',
      details: []
    });
  }
}

async function createBatchCall(request: any, reply: any) {
    ...
    if (error instanceof ValidationError) {
      return reply.status(400).send({
        ...
        suggestions: [
          ...
          'Client ID should be a valid number',
          'Remove empty rows and duplicate phone numbers/Client IDs'
        ]
      });
    }

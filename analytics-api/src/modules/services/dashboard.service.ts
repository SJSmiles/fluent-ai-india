...
import {
  ...
  NO_ANSWER_LEAD_STATUSES,
  SUCCESS_STATUSES,
  LEAD_STATUS_GROUPS,
  ANSWER_CALLS,
  NO_ANSWER_CALLS,
  LEAD_STATUSES_FOR_PM_QUALI,
  PM_QUALI_COMPANY_ID,
  ...
  public static async getAnalytics(
    ...
    switch (type) {
      ...
      case 1: {
        ...
        const currentRes = await Call.aggregate(currentPipeline);
        ...
        const previousRes = await Call.aggregate(previousPipeline);
      ...
      case 2: {
        ...
        const res = await Call.aggregate(pipeline);
      ...
      // Type 4 & 5: Status counts -> count where status in statusCodes
      case DASHBOARD_ANALYTICS_TYPE.FAILED_CALLS:
      case DASHBOARD_ANALYTICS_TYPE.ENDED_CALLS:
      case 4:
      case 5: {
        if (!statusCodes || !fieldKey) {
          throw new Error(
            "Missing statusCodes or fieldKey for status analytics"
          );
        }

        // Filter by statusCodes (status stored as number)
        pipeline.push({
          $match: {
            [fieldKey]: { $in: statusCodes },
          },
        });
        ...
        const res = await Call.aggregate(pipeline);
        const value = res[0]?.count ?? 0;
        return { data: { value } };
      }

      // Type 6: Lead Status → normalize variations like
      // “Interested Task” == “Interested - Task”, "Interested Meeting" == "Interested - Meeting"
      ...
      case 8: {
        ...
        const uniqueCallsRes = await Call.aggregate(uniqueCallsPipeline);
        ...
        const attemptsRes = await Call.aggregate(totalAttemptsPipeline);
      ...
      case 9: {
        ...
        const totalCallsRes = await Call.aggregate(totalCallsPipeline);
        ...
        const answeredRes = await Call.aggregate(answeredPipeline);
        ...
        const previousAnsweredRes = await Call.aggregate(
      ...
      case 10: {
        ...
        const noAnswerRes = await Call.aggregate(noAnswerPipeline);
      ...
      case 11: {
        ...
        const durationRes = await Call.aggregate(durationPipeline);
      ...
      case 12: {
        ...
        const totalCallsRes = await Call.aggregate(uniqueCallsPipeline);
        ...
        const successCallsRes = await Call.aggregate(successCallsPipeline);
        ...
        const previousSuccessRes = await Call.aggregate(
      ...
      case 13: {
        ...
        const statusRes = await Call.aggregate(statusPipeline);
        ...
        const STATUS_LIST =
          userDetails?.companyId.toString() === PM_QUALI_COMPANY_ID
            ? LEAD_STATUSES_FOR_PM_QUALI
            : LEAD_STATUSES_COMMONS;
